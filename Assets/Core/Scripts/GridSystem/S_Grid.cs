...
public class Grid : MonoBehaviour
{
    ...
    [SerializeField] private S_FogData _fogData = default;
    ...
    {
        _fogData.fogGridsUsageStatement.Clear();
        _fogData.fogGridsUsageStatement = S_StaticFunc.Create2DimensionalList(_mapSphereArea * 2 / _tileSize + 1 + (_padding * 2), () => false);
        ...
        for (int i = 0; i < _tileAmount; i++)
        {
            for (int j = 0; j < _tileAmount; j++)
            {
                ...
                {
                    _fogData.fogGridsUsageStatement[i][j] = true;
                }
            }
        }
    }
    ...
    void CreateFogGameObjects()
    {
        for(int i = 0; i < _tileAmount; i++)
        {
            ...
            for (int j = 0;j < _tileAmount; j++)
            {
                if (_fogData.fogGridsUsageStatement[i][j] == true)
                {
